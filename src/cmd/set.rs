...
impl Set {
    ...
    pub(crate) fn parse_frames(parse: &mut Parse) -> crate::Result<Set> {
        ...
        match parse.next_string() {
            ...
            Ok(s) if s.to_uppercase() == "NX" => {
                // Only if the key does not exist, do `set` operation
                need_exist = Some("NX".to_string());
            }
            Ok(s) if s.to_uppercase() == "XX" => {
                // Only if the key exists, do `set` operation
                need_exist = Some("XX".to_string());
            }
            ...
            Ok(_) => {
                return Err("currently `SET` only supports the [EX/PX] and [NX/XX] option".into())
            }
        ...
        if need_match {
            match parse.next_string() {
                ...
                Ok(_) => {
                    return Err(
                        "currently `SET` only supports the [EX/PX] and [NX/XX] option".into(),
                    )
                }
                // The `EndOfStream` error indicates there is no further data to
                // parse. In this case, it is a normal run time situation and
    ...
    pub(crate) async fn apply(self, db: &Db, dst: &mut Connection) -> crate::Result<()> {
        ...
        let response;
        if let Err(s) = db.set(self.key, self.value, self.expire, self.need_exist) {
