...
impl Db {
    ...
    pub(crate) fn set(
        ...
        let exist = state.entries.contains_key(&key);

        if let Some(s) = need_exist {
            if s == "NX" && exist {
                drop(state);
                return Err("set NX but the key exists".into());
            } else if s == "XX" && !exist {
                drop(state);
                return Err("set XX but the key does not exists".into());
            }
        }
    ...
    pub(crate) fn del(&self, key: &str) -> crate::Result<()> {
        // Acquire the lock, get the entry and clone the value.
        //
        // Because data is stored using `Bytes`, a clone here is a shallow
        // clone. Data is not copied.
        ...
        if let Some(v) = state.entries.remove(key) {
            ...
            return Err(format!("key \"{}\" does not exist", key).into());
        }
    }
